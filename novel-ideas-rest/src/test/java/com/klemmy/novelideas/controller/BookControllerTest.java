...
class BookControllerTest {
  ...
  void getAll__withNoInputs__success() throws Exception {
    ...
    Page<BookDto> dtoList = new PageImpl<>(List.of(
        ...
        TestEntities.bookDtoBuilder2().build()), pageable, 2);
  ...
  void getAll__withInputs__success() throws Exception {
    Pageable pageable = PageRequest.of(0, 5);
    Page<BookDto> dtoList = new PageImpl<>(Collections.singletonList(TestEntities.bookDtoBuilder().build()), pageable, 1);
  ...
  void create__inValidData__failure() throws Exception {
    ...
    this.mockMvc.perform(post("/book/")
        ...
        .andExpect(jsonPath("$.detail").value("create.bookDto.id: must be null")) // Normal Message
        .andExpect(jsonPath("$.['create.bookDto.id']").value("must be null"))  // ProblemDetail.setProperty
        .andExpect(jsonPath("$.instance").value("/book/"));
  ...
  void update__nullId__failure() throws Exception {
    ...
    this.mockMvc.perform(put("/book/")
        ...
        .andExpect(content().contentType(MediaType.APPLICATION_PROBLEM_JSON_VALUE))
        .andExpect(result -> assertThat(result.getResolvedException()).isInstanceOf(ConstraintViolationException.class))
        .andExpect(jsonPath("$.detail").value("update.bookDto.id: must not be null")) // Normal Message
        .andExpect(jsonPath("$.['update.bookDto.id']").value("must not be null"))  // ProblemDetail.setProperty
        .andExpect(jsonPath("$.instance").value("/book/"));
